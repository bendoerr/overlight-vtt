...
export class RollChromaTestDialog {
  private static html = "systems/overlight-vtt/templates/dialogs/roll-chroma-test.html" as const;

  private static getTemplate(): Promise<HTMLElement> {
    return renderTemplate(RollChromaTestDialog.html, {
      virtues: OVERLIGHT_VIRTUES,
      skills: OVERLIGHT_VIRTUES,
  ...
  constructor() {
    this.dialog = RollChromaTestDialog.getTemplate().then((content) => {
      return new Dialog(
        {
          title: game.i18n.localize("overlight-vtt.dialog.roll-chroma-test.title"),
          content: content,
          buttons: {
            close: {
  ...
  handleResponse(formData: FormData) {
    ...
    // Send it to chat
    new OverlightChromaTestChat(test).send();
  }
}
