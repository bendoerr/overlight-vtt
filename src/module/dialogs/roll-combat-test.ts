...
export class RollCombatTestDialog {
  ...
  private static getTemplate(): Promise<HTMLElement> {
    return renderTemplate(RollCombatTestDialog.html, {
  ...
  constructor() {
    this.dialog = RollCombatTestDialog.getTemplate().then((content) => {
      return new Dialog(
        {
          title: game.i18n.localize("overlight-vtt.dialog.roll-combat-test.title"),
          content: content,
          buttons: {
            close: {
  ...
  handleResponse(formData: FormData) {
    // Create a new test
    const test = new OverlightCombatTest(
      new OverlightHand(
        "skill",
        "virtue",
    ...
    // Send it to chat
    new OverlightCombatTestChat(test).send();
  }
}
