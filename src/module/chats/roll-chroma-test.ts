...
export class OverlightChromaTestChat {
  ...
  private static getTemplate(data: OverlightChromaTestResult): Promise<HTMLElement> {
    return renderTemplate(OverlightChromaTestChat.html, data);
  }
  ...
  async send() {
    return OverlightChromaTestChat.getTemplate(this.test.result()).then(async (content) => {
      if (game.dice3d) {
  ...
  resolveTestName(): string {
    return game.i18n.format("overlight-vtt.tests.chroma.name", {});
  }
  ...
  resolveSkillName(): string {
    return game.i18n.format("overlight-vtt.virtue." + this.test.hand.poolTwo.name + ".name", {});
  }
